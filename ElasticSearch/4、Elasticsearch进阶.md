...
![img](4、Elasticsearch进阶.assets/146a779da01f53e7f7a8d53132d3c7cf.png)
...
![img](4、Elasticsearch进阶.assets/e4d13427545dc174eb9ccface85c1f0c.png)
...
![img](4、Elasticsearch进阶.assets/54ee6753be248cc7d345b38a0eae7d96.png)

通过 elasticsearch-head 插件（一个Chrome插件）查看集群情况 。

![img](4、Elasticsearch进阶.assets/e8b15d0b243d486e91f478a220da63bf.png)

- 集群健康值:yellow( 3 of 6 )：表示当前集群的全部主分片都正常运行，但是副本分片没有全部处在正常状态。

![img](4、Elasticsearch进阶.assets/489b6de480112879a00067b793bde685.png)：3 个主分片正常。

![img](4、Elasticsearch进阶.assets/3ce9a78d26ee762f0a7a8abf7817a58e.png)：3 个副本分片都是 Unassigned，它们都没有被分配到任何节点。 在同 一个节点上既保存原始数据又保存副本是没有意义的，因为一旦失去了那个节点，我们也将丢失该节点 上的所有副本数据。
...
![img](4、Elasticsearch进阶.assets/bf76cb1bfbdf07555918d9055817ab44.png)

通过 elasticsearch-head 插件查看集群情况

![img](4、Elasticsearch进阶.assets/18db400822b83e727d6206f486b7b2ea.png)

- 集群健康值:green( 3 of 6 )：表示所有 6 个分片（包括 3 个主分片和 3 个副本分片）都在正常运行。

- ![img](4、Elasticsearch进阶.assets/e485d8263a4aa3a94af0be951bd5a241.png)：3 个主分片正常。
- ![img](4、Elasticsearch进阶.assets/e485d8263a4aa3a94af0be951bd5a241-1673924176584-31.png)：第二个节点加入到集群后， 3 个副本分片将会分配到这个节点上——每 个主分片对应一个副本分片。这意味着当集群内任何一个节点出现问题时，我们的数据都完好无损。所 有新近被索引的文档都将会保存在主分片上，然后被并行的复制到对应的副本分片上。这就保证了我们 既可以从主分片又可以从副本分片上获得文档。
...
![img](4、Elasticsearch进阶.assets/d527e26aa2bccdf54b11410024eadc92.png)

通过 elasticsearch-head 插件查看集群情况。

![img](4、Elasticsearch进阶.assets/6985fe14454c1269204478320d089bd7.png)

- 集群健康值:green( 3 of 6 )：表示所有 6 个分片（包括 3 个主分片和 3 个副本分片）都在正常运行。

- ![img](4、Elasticsearch进阶.assets/9494419153adb44bedb395ac5d7bc488.png)Node 1 和 Node 2 上各有一个分片被迁移到了新的 Node 3 节点，现在每个节点上都拥有 2 个分片， 而不是之前的 3 个。 这表示每个节点的硬件资源（CPU, RAM, I/O)将被更少的分片所共享，每个分片 的性能将会得到提升。
...
![img](4、Elasticsearch进阶.assets/97fd01e34e5d8df23d226c4fef157801.png)

通过 elasticsearch-head 插件查看集群情况：

![img](4、Elasticsearch进阶.assets/8bf9dbf0cec5b7875bf8aa9d17a9a67c.png)
...
![img](4、Elasticsearch进阶.assets/44e841004be934e6bce08187ca3852bb.png)
...
![img](4、Elasticsearch进阶.assets/e956bda7e0d005699e27760d4193d101.png)
...
![img](4、Elasticsearch进阶.assets/37eea6a8dae7ba908312f2ebf0eced11.png)
...
![img](4、Elasticsearch进阶.assets/9c34e8603887c2bed475416e3b67cd9a.png)
...
![img](4、Elasticsearch进阶.assets/3940d6cdb197259368542b86384911a4.png)
...
![img](4、Elasticsearch进阶.assets/418356a32516c222a8d366df021276c2.png)
...
![img](4、Elasticsearch进阶.assets/7139df83ee6f7a59c5d3252d34cc8762.png)
...
![img](4、Elasticsearch进阶.assets/ca41993144aee98311671278725437cd.png)
...
![img](4、Elasticsearch进阶.assets/b90ea9c79138d8361ca339cff205fdb0.png)
...
![img](4、Elasticsearch进阶.assets/83499315a7b8ab81471a88f3e142f0a8.png)
...
![img](4、Elasticsearch进阶.assets/cba02cc6d7c5f054dfe5d58fafac9a6a.png)
...
![img](4、Elasticsearch进阶.assets/a1f52e96e0ac218b5024d708202afba4.png)
...
![img](4、Elasticsearch进阶.assets/3cc642e9bae776c3e617f9d117d41e21.png)

现在，如果我们想搜索 `quick` `brown` ，我们只需要查找包含每个词条的文档：

![img](4、Elasticsearch进阶.assets/f26aaa01e011edfa68736956b2f1ddea.png)
...
![img](4、Elasticsearch进阶.assets/19813d1918c89461303377444cf85c8c.png)
...
![img](4、Elasticsearch进阶.assets/9ee1adbb2d55e710257e01b812a6d8cf.png)

按段搜索会以如下流程执行：

一、新文档被收集到内存索引缓存。

![img](4、Elasticsearch进阶.assets/9d499fde966ee9825fa5a424d8357489.png)
...
![img](4、Elasticsearch进阶.assets/f74828ff58cc4635a97e88706a221e50.png)
...
![img](4、Elasticsearch进阶.assets/b3b31c1e592d5aa794e7c9fcb259c924.png)

![img](4、Elasticsearch进阶.assets/521c25f0f16247240234d1b8eb3c5f25.png)
...
![img](4、Elasticsearch进阶.assets/a679d4f5f4bfa6913a53316251beef2a.png)

Lucene允许新段被写入和打开，使其包含的文档在未进行一次完整提交时便对搜索可见。这种方式比进行一次提交代价要小得多，并且在不影响性能的前提下可以被频繁地执行。

![img](4、Elasticsearch进阶.assets/673d3a77e254fa3a5a6f5293ffb125ab.png)
...
![img](4、Elasticsearch进阶.assets/baeab48c8d6b87660ac4fb954e9c9731.png)
...
![img](4、Elasticsearch进阶.assets/17be4247e6b23f31b1e589c70d61e817.png)



三、这个进程继续工作，更多的文档被添加到内存缓冲区和追加到事务日志。

![img](4、Elasticsearch进阶.assets/4b5c4a3a3ffb4c84625bb283f6a67018.png)
...
![img](4、Elasticsearch进阶.assets/11c7d2cc05244e669eb8402dd8049de9.png)
...
![img](4、Elasticsearch进阶.assets/c907ca35bd7c0393d46aec2c7038af19.png)
...
![img](4、Elasticsearch进阶.assets/a00cc1c19652c47fcfb663aaf337a41b.png)
...
![img](4、Elasticsearch进阶.assets/49ca2ec50db3ddd0fcd1f364ac600b96.png)
...
![img](4、Elasticsearch进阶.assets/d066ba5e26916624b5d056d04c5580ac.png)