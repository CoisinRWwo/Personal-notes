...
可以在**创建表时规定约束（通过 CREATE TABLE 语句），或者在表创建之后通过 ALTER TABLE 语句规定 约束**。
...
![image-20220816101200779](13、约束.assets/image-20220816101200779.png)
...
![image-20220816101507703](13、约束.assets/image-20220816101507703.png)
...
![image-20220816101753595](13、约束.assets/image-20220816101753595.png)
...
![image-20220816111948930](13、约束.assets/image-20220816111948930.png)
...
AUTO_INCREMENT=max(primary key)+1，这种现象在某些情况下会导致业务主键冲突或者其他难以发 现的问题。 下面通过案例来对比不同的版本中自增变量是否持久化。 在MySQL 5.7版本中，测试步骤如 下： 创建的数据表中包含自增主键的id字段，语句如下：
...
![image-20220816201646604](13、约束.assets/image-20220816201646604.png)
...
not null No action方式 ：如果子表中有匹配的记录，则不允许对父表对应候选键进行

update/delete操作 Restrict方式 ：同no action， 都是立即检查外键约束 
...
答:（1）不好比较。null是一种特殊值，比较时只能用专门的is null 和 is not null来比较。碰到运算符，通 常返回null。 （2）效率不高。影响提高索引效果。因此，我们往往在建表时 not null default '' 或 default 0 
